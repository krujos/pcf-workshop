...
. Review the keynote on link:BuildpacksAndServices.key[Buildpacks and Service Brokers]
...
=== Push your changes to your buildpack fork

. Be sure to commit your changes to your forked buildpack repo (not the class workspace).

== Checking your work

. A buildpack named <first-initial><last-initial>-java-buildpack should show as a registered buildpack:
+
[source,bash]
----
$ cf buildpacks
...
. Your cities-service manifest should specify the above buildpack.

. A curl request should return the staging timestamp:
...
. The buildpack changes should be pushed to your java buildpack fork.
