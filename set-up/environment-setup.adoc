...
* Map the pfSense interfaces to the VM network NICs
+
** Identify and write down the MAC address for each network NIC (available by Ctrl + click on the VM > Edit Settings)
** Match the MAC address to the pfSense interface (le0 or le1) displayed on the pfSense console in the step below
...
** Enter WAN interface name (le0 or le1): Select the NIC on the WAN network (available by Ctrl + click on the VM > Edit Settings)
** Enter LAN interface name (le0 or le1): Select the NIC on the LAN network (available by Ctrl + click on the VM > Edit Settings)
